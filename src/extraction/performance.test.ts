...
async function runSinglePerformanceTest(
  ...
  const throughputUrlsPerSecond = Math.round((extractedCount / extractionTimeMs) * 1000)
...
async function runPerformanceTests(): Promise<PerformanceResult> {
  ...
  for (const { name, format } of PERFORMANCE_FILES) {
    ...
    try {
      ...
      console.log(`  ✓ Extracted ${metric.extractedCount.toLocaleString()} URLs`)
      console.log(`  ✓ Throughput: ${metric.throughputUrlsPerSecond.toLocaleString()} URLs/sec`)
      console.log('')
    } catch (error) {
